$(function() {
  ...
  //  lodash task starts here
 var dataObj =  [
  {
    "guid": "701b29fdf-b35d-4cf1-a5f6-8b12b29a5081",
    "index": 0,
    "favoriteFruit": "banana",
    "latitude": 58.16152499999997,
    "company": "INDEXIA",
  ...
  },
  {
    ...
    "gender": "female",
    "age": 39,
    "greeting": "Hello, Sheree Anthony! You have 5 unreafdsfd messages.",
    "longitude": 37.385548,
    "_id": "56bdc3396fdbaecdfddfd5ee4cas1929"
 }
 ];



console.log( "Object data.json = ", dataObj);

var skillsFinder = function() {
     var skills = dataObj.map(function (item) {
        return item.skills;
    });
    console.log("all skills arrays = ", skills);

    var allSkills = skills.join(",");
    console.log("allSkills joined = ", allSkills);

    var allSkillsArr = allSkills.split(",");
    console.log(allSkillsArr.length);

        for (var i = 0; i < allSkillsArr.length; i++) {
            console.log(i, allSkillsArr[i]);
        };
        ...
        function unique(arr) {
            ...
            for (var i = 0; i < arr.length; i++) {
                var str  = allSkillsArr[i];
                obj[str] = true;
            }
            return Object.keys(obj);
        }

        console.log("sorted unique arr = ", unique(allSkillsArr));

}
console.log(skillsFinder());
...
var friendsSorting = _.map(_.orderBy(data, 'friends.length'), 'name');
console.log("Rating by friends amount = ", friendsSorting);


// all friends of all users
    friends = data.map(function (item) {
        return item.friends;
    });

    allFriendsList = [];
    for (var index in friends) {
        for (var index2 in friends[index]) {
            allFriends = friends[index][index2].name;
            console.log(allFriends);
            allFriendsList.push(allFriends);
        }
    }
    console.log("allFriendsList = ", allFriendsList);
    allUniqFriends = _.uniq(allFriendsList);
    console.log("all uniq friends = ", allUniqFriends);
